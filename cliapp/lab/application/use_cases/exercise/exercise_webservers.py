...
# lab/application/use_cases/exercise/exercise_webservers.py
...
class ExerciseWebServers:
    ...
    def _create_playbook(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            failed = True
            error_output = get_text(LANG,'error_create_webservers_yml')
        return failed, error_output
    ...
    def start(self, notifier: ProgressNotifierPort) -> None:
        ...
        event_info = EventInfo(name=get_text(LANG,'creando_role_apache'))
        spinner_handle, finished_event = notifier.start(event_info)
        ...
        event_info = EventInfo(name=get_text(LANG,'limpiando_carpetas_apache'))
        spinner_handle, finished_event = notifier.start(event_info)
        ...
        event_info = EventInfo(name=get_text(LANG,'creando_role_nginx'))
        spinner_handle, finished_event = notifier.start(event_info)
        ...
        event_info = EventInfo(name=get_text(LANG,'limpiando_carpetas_nginx'))
        spinner_handle, finished_event = notifier.start(event_info)
        ...
        event_info = EventInfo(name=get_text(LANG,'creacion_playbook_webservers'))
        spinner_handle, finished_event = notifier.start(event_info)
    ...
    def finish(self, notifier: ProgressNotifierPort) -> None:
        ...
        container_service = ContainerAdapter()
        container_service.init_client()
        event_info = EventInfo(name=get_text(LANG,'eliminando_container_web1'))
        spinner_handle, finished_event = notifier.start(event_info)
